...
public class MessageBusAdapterConfigurationTests {
	...
	public void oneOutput() throws Exception {
		...
		refresh();
		Collection<OutputBinding> channels = this.adapter.getChannelsMetadata().getOutputBindings();
		assertEquals(1, channels.size());
		assertEquals("group.0", channels.iterator().next().getPipeName());
		assertEquals("tap:stream:group.module.0", channels.iterator().next().getTapChannelName());
	}

	private void refresh() {
		Collection<OutputBinding> bindings = this.configuration.getOutputBindings();
		for (OutputBinding binding : bindings) {
			binding.setTapped(true);
		}
		this.adapter.setOutputBindings(bindings);
		this.adapter.start();
	}
	...
	public void oneOutputTopic() throws Exception {
		...
		Collection<OutputBinding> channels = this.adapter.getChannelsMetadata().getOutputBindings();
		assertEquals(1, channels.size());
		assertEquals("topic:group.0", channels.iterator().next().getPipeName());
		assertEquals("tap:stream:group.module.0", channels.iterator().next().getTapChannelName());
	}
	...
	private List<String> getChannelNames(Collection<? extends InputBinding> channels) {
		List<String> list = new ArrayList<String>();
		for (InputBinding binding : channels) {
			list.add(binding.getPipeName());
		}
		return list;
	}
	...
	public void overrideNaturalOutputChannelName() throws Exception {
		...
		Collection<OutputBinding> channels = this.adapter.getChannelsMetadata().getOutputBindings();
		assertEquals(1, channels.size());
		assertEquals("foo.bar", channels.iterator().next().getPipeName());
	...
	public void overrideNaturalOutputChannelNamedQueueWithTopic() throws Exception {
		...
		Collection<OutputBinding> channels = this.adapter.getChannelsMetadata().getOutputBindings();
		assertEquals(1, channels.size());
		assertEquals("topic:foo.bar", channels.iterator().next().getPipeName());
		assertEquals("tap:stream:foo.bar.module.0", channels.iterator().next().getTapChannelName());
	}
